...
func NewAddHeader(modName, instName string, _, inlineArgs []string) (module.Module, error) {
	...
	if len(inlineArgs) < 1 {
		return nil, fmt.Errorf("missing argument header_name")
	}
	if len(inlineArgs) < 2 {
		return nil, fmt.Errorf("missing argument header_value")
	}

	r := addHeader{
		modName:     modName,
		instName:    instName,
		headerName:  inlineArgs[0],
		headerValue: inlineArgs[1],
	}

	return &r, nil
}

func (r *addHeader) Init(cfg *config.Map) error {
	return nil
}
